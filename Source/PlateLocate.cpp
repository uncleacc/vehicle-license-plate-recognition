...
//验证图像蓝色面积占比超过3成（如此低是因为有些图片截取的矩形太大了）
bool PlateLocate::verifyArea(Mat &src) {
    int n = src.rows, m = src.cols, dim = src.channels();
    //利用访问某个像素点的函数
    int cnt = 0;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (dim == 3) { // 三通道
                ...
                if(blue > green + 10 && blue > red + 10) ++ cnt;
            }
        }
    }
//    if(num == 2) cout << cnt << " " << n*m << endl;
    if(cnt * 1.0 / (n * m) > 0.3) return true;
    return false;
}